...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    dim = w.shape[0] # weight dimension
    xr = x.reshape(-1, dim)  # N * D resphare input to dim
    out = xr.dot(w) + b   #forward output x . w +bias 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    dx = np.dot(dout, w.T)           # derivative out . weight =dx
    dx = np.reshape(dx, x.shape)
    xRow = x.reshape(x.shape[0], -1) # (N,D)
    dw = np.dot(xRow.T, dout)       # dw weight = input 
...
def relu_backward(dout, cache):
    ...
    ###########################################################################
    x = np.maximum(x, 0)
    delta = np.zeros_like(x)  # return  x zero array 
    delta[x>0] =1  # set delta 
    dx = dout * delta  
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
